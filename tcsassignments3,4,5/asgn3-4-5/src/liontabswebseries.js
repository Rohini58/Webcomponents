...
export class LionTabsWebseries extends LitElement {
  static styles = css`
    :host {
      display: block;
      font-family: Arial;
  ...
  firstUpdated(changedProperties) {
    ...
    if (!customElements.get('lion-tabs')) {
      customElements.define('lion-tabs', LionTabs);
    }

    const tabs = this.shadowRoot.querySelector('lion-tabs');
    tabs.addEventListener('selected-changed', event => {
      // Handle tab selection change here
      console.log('Selected tab index:', event.detail.selectedIndex);
    });
  }
